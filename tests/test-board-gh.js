...
// --- add-milestone / close-milestone / move on goals (Director-only, journaled) ----
{
  ...
  // 0.12 migration surface: put the ordinal on a milestone titled under an older grammar.
  st.milestones.push({ number: 90, title: 'C7 legacy title', description: '', state: 'open', open_issues: 0, closed_issues: 0 });
  r = run(['retitle-milestone', '90', '--title', 'M7 · legacy title'], gh);
  check('retitle-milestone puts the ordinal on a legacy title, by number', r.code === 0 && st.milestones.find(m => m.number === 90).title === 'M7 · legacy title' && /#90 → M7 · legacy title/.test(r.out));
  r = run(['retitle-milestone', '90', '--title', 'M7 · legacy title'], gh);
  check('retitle-milestone is idempotent', r.code === 0 && /already/.test(r.out));
  r = run(['retitle-milestone', '90', '--title', 'C7 legacy title'], gh);
  check('retitle-milestone refuses a title without the M<n> · ordinal', r.code === 1 && /program ordinal/.test(r.out));
  r = run(['retitle-milestone', '90', '--title', 'M1 · Ship the HMI'], gh);
  check('retitle-milestone refuses an ordinal another milestone already carries', r.code === 1 && /already carries/.test(r.out));
  r = run(['retitle-milestone', '999', '--title', 'M8 · nope'], gh);
  check('retitle-milestone names an unknown milestone', r.code === 1 && /no milestone/.test(r.out));
  r = run(['retitle-milestone', '90', '--title', 'M7 · x'], gh, { env: { ORCH_ROLE: 'coordinator' } });
  check('retitle-milestone is Director-only', r.code === 1 && /refused/.test(r.out));
  st.milestones = st.milestones.filter(m => m.number !== 90);
