# /shepr:go — the native session (route → work → ship)

Loaded by `SKILL.md` when this pane is not a Coordinator tick. Every phase
of one interactive session lives here; `coordinator.md` never loads it.
...
| # | Condition | Phase |
|---|---|---|
| 0 | coordinator tick | routed by SKILL.md before this file loads — never reached here |
