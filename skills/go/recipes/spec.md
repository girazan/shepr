...
Fits: fuzzy or big. The Architect's pass (or `/shepr:goal` in a single session); never on a step.
...
3. **`P.R1`.** The plan review (plan 4's `shepr review G<k> --plan`); until it ships, a checker reads the plan section against the rubric adds below.
